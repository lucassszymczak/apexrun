...
// 80/20: % do tempo em zona fácil (Z1–Z2)
export function polarization(workouts, athlete) {
  let easy = 0, total = 0;
  for (const w of workouts) {
    if (!w.hrAvg || !w.durationSec) continue;
    const z = hrZone(w.hrAvg, athlete.fcRep, athlete.fcMax);
    if (z == null) continue;
    total += w.durationSec;
    if (z <= 2) easy += w.durationSec;
  }
  if (!total) return null;
  return { easyPct: (easy / total) * 100, totalSec: total };
}
...
export function readiness(ctx) {
  // ctx: { tsb, adherencePct, sleepH, soreness (0-10), pain (bool) }
  let score = 75;
